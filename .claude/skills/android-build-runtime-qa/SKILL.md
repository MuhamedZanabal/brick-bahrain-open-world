...
- Preserve package/version authority; report current inconsistencies instead of silently rewriting them.

## Build procedure

1. Validate source authority and protected hashes.
2. Run Python contracts and Godot clean import/headless validation.
3. Use the existing repository debug-export script or typed `godot_export_android_debug` tool.
4. Never pass keystore bytes, passwords, tokens, or production signing material through MCP.
5. Run `collect_build_evidence` immediately after export.
6. Use `android_adb_smoke_test` only with an explicitly selected device and debug/QA package.
