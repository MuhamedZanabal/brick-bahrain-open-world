...
# Reconstruct accepted production source while replacing only the obsolete hosted-runner assertion.
...
# Apply the production Compatibility correction before import.
...
test -s "$APK"; "$APKSIGNER" verify --verbose --print-certs "$APK" > "$OUTPUT_ROOT/apk-signing.txt"; sha256sum "$APK" > "$OUTPUT_ROOT/APK_SHA256SUMS.txt"
...
"$ADB" shell wm size 1920x1080 >/dev/null; "$ADB" shell wm density 420 >/dev/null
...
"$ADB" logcat -c; "$ADB" logcat -v threadtime > "$OUT/logcat_full.txt" 2>&1 & LOGCAT_PID=$!; sleep 2
...
elapsed=0; marker=false
while ((elapsed<300)); do grep -q 'R1_GL_SCENARIO_COMPLETE mode=gl_production' "$OUT/logcat_full.txt" && { marker=true; break; }; sleep 2; elapsed=$((elapsed+2)); done
"$ADB" exec-out screencap -p > "$OUT/screenshot.png" 2>/dev/null || true
PID="$("$ADB" shell pidof "$PACKAGE" 2>/dev/null | tr -d '\r')"; kill "$LOGCAT_PID" >/dev/null 2>&1 || true; wait "$LOGCAT_PID" >/dev/null 2>&1 || true
...
python3 - "$OUT/screenshot.png" "$OUTPUT_ROOT/R1_GL_BUDGET_RESULT.json" "$marker" "$PID" "$COUNT" "$CRITICAL" "$elapsed" <<'PY'
from pathlib import Path
