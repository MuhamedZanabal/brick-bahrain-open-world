...
OUT="$OUTPUT_ROOT/raw/mobile_tonemapper"
APK="$OUTPUT_ROOT/bahrain-brick-r1-mobile-tonemapper-x86_64.apk"
PACKAGE="com.brickbahrain.r1mobiletonemapper"
...
AVD_NAME="bahrain_brick_r1_mobile_tonemapper"
...
python3 "$REPO_ROOT/tools/graphics/apply_r1_mobile_tonemapper_fix.py" --script "$GAME/scripts/manama_souq_vertical_slice.gd" --report "$OUTPUT_ROOT/MOBILE_TONEMAPPER_FIX.json"
...
python3 - "$OUT/mobile_progress.json" "$OUT/screenshot.png" "$OUT/am-start.txt" "$OUTPUT_ROOT/R1_MOBILE_TONEMAPPER_RESULT.json" "$PID_BEFORE" "$CRITICAL" "$PAUSE_RESUME" "$SCENE_READY" "$elapsed" <<'PY'
from pathlib import Path
import json,sys
