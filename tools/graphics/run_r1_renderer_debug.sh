...
OUT="$OUTPUT_ROOT/raw/mobile_render_scale"
APK="$OUTPUT_ROOT/bahrain-brick-r1-mobile-render-scale-x86_64.apk"
PACKAGE="com.brickbahrain.r1mobilerenderscale"
...
AVD_NAME="bahrain_brick_r1_mobile_render_scale"
...
python3 "$REPO_ROOT/tools/graphics/apply_r1_mobile_render_scale_fix.py" --project "$GAME/project.godot" --report "$OUTPUT_ROOT/MOBILE_RENDER_SCALE_FIX.json"
...
python3 - "$OUT/mobile_progress.json" "$OUT/screenshot.png" "$OUT/am-start.txt" "$OUTPUT_ROOT/R1_MOBILE_RENDER_SCALE_RESULT.json" "$PID_BEFORE" "$CRITICAL" "$PAUSE_RESUME" "$SCENE_READY" "$elapsed" <<'PY'
from pathlib import Path
import json,sys
