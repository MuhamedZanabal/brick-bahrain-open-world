...
class PostLifecycleGuardTests(unittest.TestCase):
    ...
    def test_applies_parent_guard_and_preserves_protected_hash(self):
    ...
    def test_correction_is_idempotent(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual([item["state"] for item in report["changes"]], ["already_satisfied", "already_satisfied"])
